...
function Section({title, children}: {title: string, children: React.ReactNode}) {
  return (<section className="rounded-xl border p-4 mb-6"><h2 className="font-semibold text-lg mb-3">{title}</h2>{children}</section>);
}
  ...
  return (<div className="max-w-4xl mx-auto p-4">
    ...
    <Section title="Настройки сессии">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="API Base URL"><input className="border rounded px-3 py-2" value={baseUrl} onChange={e=>setBaseUrl(e.target.value)} placeholder="https://api.afterl.ru"/></Field>
        <Field label="User ID (x-user-id)"><input className="border rounded px-3 py-2" value={userId} onChange={e=>setUserId(e.target.value)} placeholder="uuid"/></Field>
      </div>
    ...
    <Section title="Health">
      <div className="flex gap-2">
        <Button disabled={busy} onClick={async ()=>{ setBusy(true); try { setOut(await api('/healthz')); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>GET /healthz</Button>
        <Button disabled={busy} onClick={async ()=>{ setBusy(true); try { setOut(await api('/readyz')); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>GET /readyz</Button>
      </div>
    </Section>
    <Section title="Vaults">
      <div className="flex flex-wrap gap-2">
        ...
        } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST /vaults (Create)</Button>
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try { setOut(await api(`/vaults/${vaultId}`)); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>GET /vaults/{'{vaultId}'}</Button>
      </div>
    </Section>
    ...
    <Section title="Blocks (multipart)">
      ...
      <div className="flex gap-2">
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try {
        ...
        } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST /blocks (Create)</Button>
        <Button disabled={busy || !blockId} onClick={async ()=>{ setBusy(true); try { setOut(await api(`/blocks/${blockId}`)); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>GET /blocks/{'{blockId}'}</Button>
      </div>
    </Section>
    <Section title="Block → Recipients">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Field label="recipient_id"><input id="br-recipient" className="border rounded px-3 py-2" value={recipientId} onChange={(e)=>setRecipientId(e.target.value)}/></Field>
        <Field label="dek_wrapped_for_recipient"><input id="br-dek" className="border rounded px-3 py-2" placeholder="base64:WRAPPED_DEK"/></Field>
      </div>
      <Button disabled={busy || !blockId || !recipientId} onClick={async ()=>{ setBusy(true); try {
        ...
        setOut(data);
      } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST /blocks/{'{blockId}'}/recipients</Button>
    </Section>
    <Section title="Public Link for Block">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Field label="enabled"><select id="pl-enabled" className="border rounded px-3 py-2" defaultValue="true"><option value="true">true</option><option value="false">false</option></select></Field>
        <Field label="publish_from (ISO)"><input id="pl-from" className="border rounded px-3 py-2" defaultValue={new Date().toISOString()}/></Field>
        <Field label="publish_until (ISO or empty)"><input id="pl-until" className="border rounded px-3 py-2" placeholder=""/></Field>
        <Field label="max_views (optional)"><input id="pl-max" className="border rounded px-3 py-2" placeholder="10"/></Field>
      </div>
      <Button disabled={busy || !blockId} onClick={async ()=>{ setBusy(true); try {
        ...
        setOut(data);
      } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>PUT /blocks/{'{blockId}'}/public</Button>
    </Section>
    <Section title="Verifiers & Orchestrator">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Field label="Invite verifier (contact)"><input id="inv-contact" className="border rounded px-3 py-2" defaultValue="verifier1@example.com"/></Field>
        <Field label="channel"><select id="inv-channel" className="border rounded px-3 py-2" defaultValue="email"><option value="email">email</option><option value="sms">sms</option></select></Field>
        <Field label="expires_in_hours"><input id="inv-expires" className="border rounded px-3 py-2" defaultValue="168"/></Field>
      </div>
      <div className="flex flex-wrap gap-2">
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try {
          ...
          setOut(data);
        } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST /verifiers/invitations</Button>
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try { setOut(await api('/orchestration/start', { method: 'POST', body: JSON.stringify({ vault_id: vaultId }) })); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST /orchestration/start</Button>
        <Button disabled={busy || !vaultId || !verifierId} onClick={async ()=>{ setBusy(true); try {
          ...
          setOut(data);
        } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST decision Confirm</Button>
        <Button disabled={busy || !vaultId || !verifierId} onClick={async ()=>{ setBusy(true); try {
          ...
          setOut(data);
        } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST decision Deny</Button>
      </div>
    </Section>
    <Section title="Heartbeat">
      <div className="flex flex-wrap gap-2">
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try { setOut(await api(`/vaults/${vaultId}/heartbeat`)); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>GET /vaults/{'{vaultId}'}/heartbeat</Button>
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try { setOut(await api(`/vaults/${vaultId}/heartbeat`, { method: 'PATCH', body: JSON.stringify({ method: 'manual', timeout_days: 60 }) })); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>PATCH /vaults/{'{vaultId}'}/heartbeat</Button>
        <Button disabled={busy || !vaultId} onClick={async ()=>{ setBusy(true); try { setOut(await api('/heartbeats/ping', { method: 'POST', body: JSON.stringify({ vault_id: vaultId, method: 'manual' }) })); } catch(e:any){ setOut(e); } finally { setBusy(false); } }}>POST /heartbeats/ping</Button>
      </div>
    </Section>
